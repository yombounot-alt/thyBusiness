...
class OfflineReadInterceptor extends Interceptor {
  ...
  bool _isReadable(RequestOptions o) {
    ...
    return _cacheablePrefixes.any(o.path.startsWith);
  }

  String? _scopeFor(RequestOptions o) => o.path == '/auth/me' ? sessionScope : scope();
  ...
  Future<void> onError(DioException err, ErrorInterceptorHandler handler) async {
    ...
    try {
      final entry = await cache.get(scopeKey, cacheKeyFor(options)) ?? await _derive(scopeKey, options);
      if (entry != null) {
  ...
  Future<CachedEntry?> _derive(String scopeKey, RequestOptions o) async {
    if (o.path == '/products' || o.path == '/products/low-stock') {
      ...
      final matches = items.where((p) {
        if (categoryId != null && p['categoryId'] != categoryId) return false;
        if (search.isEmpty) return true;
        return ['name', 'sku', 'barcode'].any(
          (field) => ((p[field] as String?) ?? '').toLowerCase().contains(search),
        );
      }).toList();
    ...
    if (o.path == '/customers') {
      ...
      final matches = items.where((c) {
        if (search.isEmpty) return true;
        return ['fullName', 'phone'].any(
          (field) => ((c[field] as String?) ?? '').toLowerCase().contains(search),
        );
      }).toList();
      return CachedEntry(data: _page(matches), savedAt: base.savedAt);
    }
