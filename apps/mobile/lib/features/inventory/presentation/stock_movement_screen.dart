...
class _MovementFormState extends ConsumerState<_MovementForm> {
      ...
      case _MovementMode.count:
        ...
        if (gap == 0) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Aucun écart : le stock est déjà exact.')),
          );
          context.pop();
          return;
        }
  ...
  Widget _countPreview() {
    ...
    final gapText = gap == 0 ? 'aucun écart' : 'écart de ${gap > 0 ? '+' : '−'}${formatQuantity(gap.abs())}';
    return Text(
      'Stock théorique : ${formatQuantity(_product.currentStock)} '
