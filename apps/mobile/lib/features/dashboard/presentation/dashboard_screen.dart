    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            Text('Bonjour, ${profile.fullName.split(' ').first} 👋', style: textTheme.headlineMedium),
...
class _SummarySection extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final profitColor = summary.profit < 0 ? AppColors.danger : AppColors.success;
    ...
    return Column(
      children: [
          ...
          children: [
            ...
            Expanded(
              child: _KpiCard(
                ...
                value: formatMoney(summary.profit, currency: currency),
        ...
        _SectionCard(
          title: 'Alertes',
          child: _Alerts(summary: summary, currency: currency),
        ),
        const SizedBox(height: 16),
        _SectionCard(
          ...
          child: Column(
            children: [
              ...
              _FinanceRow('Coût des marchandises', _deduction(summary.cogs, currency)),
              _FinanceRow('Dépenses', _deduction(summary.expensesTotal, currency)),
              const Divider(),
              _FinanceRow(
                'Bénéfice estimé',
                formatMoney(summary.profit, currency: currency),
                bold: true,
                color: profitColor,
              ),
            ],
  ...
  static String _deduction(num amount, String currency) =>
      amount.round() == 0 ? formatMoney(0, currency: currency) : '−${formatMoney(amount, currency: currency)}';
}
    ...
    final rows = <Widget>[
      ...
      if (summary.outstandingCredits > 0)
        _AlertRow(
          ...
          text: 'Créances en cours : ${formatMoney(summary.outstandingCredits, currency: currency)}',
          onTap: () => context.push('/credits'),
        ),
    ];
