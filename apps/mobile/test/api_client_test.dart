...
import 'package:thy_business/core/offline/offline_cache.dart';
...
DioException _noNetwork(RequestOptions o) =>
    DioException(requestOptions: o, type: DioExceptionType.connectionError, error: 'no route to host');
...
const _profile = {
  ...
  'businesses': [
    {'id': 'biz-1', 'name': 'Boutique Demo', 'currency': 'GNF', 'role': 'owner'},
  ],
};
...
void main() {
  ...
  group('keeping people signed in', () {
    test('an expired access token on /auth/me is renewed and the request retried', () async {
      ...
      final response = await clientWith(network).dio.get('/auth/me');
    ...
    test('a refresh that cannot reach the server does NOT end the session', () async {
      ...
      await expectLater(
        clientWith(network).dio.get('/products'),
        throwsA(isA<DioException>().having((e) => isConnectionFailure(e), 'is a network failure', true)),
      );
    ...
    test('wrong credentials at login are not mistaken for an expired session', () async {
      final network = _FakeNetwork((o) async => _json({'message': 'Identifiants invalides'}, status: 401));

      await expectLater(
        clientWith(network).dio.post('/auth/login', data: {'phone': 'x', 'password': 'y'}),
  ...
  group('offline reads', () {
    ...
    test('the profile is available offline at start-up, before anyone is "in scope"', () async {
      ...
      await settle();
      online = false;
      final offline = await dio.get('/auth/me');
    ...
    test('photos (binary answers) are not cached', () async {
      ...
      await dio.get<List<int>>('/products/p1/image', options: Options(responseType: ResponseType.bytes));
      await settle();
    ...
    test('says whether the server could be reached', () async {
      ...
      mode = 'error';
      await dio.post('/sales', data: {}).catchError((_) => Response(requestOptions: RequestOptions()));
    ...
    group('searching the catalogue offline', () {
        ...
        network = _FakeNetwork((o) async {
          ...
          return _json(
            _page([
              ...
              _product('p3', 'Sucre en poudre', categoryId: 'alimentaire', stock: '4', threshold: '10'),
      ...
      Future<List<String>> names(Map<String, dynamic> query) async {
        final r = await dio.get('/products', queryParameters: {...query, 'page': 1, 'pageSize': 100});
      ...
      test('inactive products cannot be listed offline (the cache only holds active ones)', () async {
        await expectLater(
    ...
    test('customers can be searched offline too (to sell on credit)', () async {
      ...
      final byName = await dio.get('/customers', queryParameters: {'search': 'mama', 'page': 1, 'pageSize': 100});
      final byPhone = await dio.get('/customers', queryParameters: {'search': '6220', 'page': 1, 'pageSize': 100});
