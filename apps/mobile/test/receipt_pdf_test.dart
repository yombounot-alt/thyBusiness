    ...
    for (var i = 1; i <= items; i++)
      {
        ...
        'productNameSnapshot': i.isOdd ? 'Eau minérale 1,5 L — pack économique n°$i' : 'Crème à l\'ail $i',
        'unitPrice': '5000',
        'quantity': '2',
        'lineTotal': '10000',
      },
...
void main() {
  ...
  group('buildReceiptPdf', () {
    test('produces a real, single-page PDF with the font embedded', () async {
      ...
      final box = RegExp(r'/MediaBox\s*\[\s*0\s+0\s+([\d.]+)\s+([\d.]+)\s*\]').firstMatch(_latin1(pdf));
    ...
    test('builds every variant: cash, credit with customer, discount, voided, no address', () async {
      final variants = <Sale>[
        _sale(paid: 20000),
  ...
  group('Reçu en PDF button', () {
    ...
    setUp(() {
      final catalog = [testProduct(id: 'p1', name: 'Eau minérale 1.5L', stock: 10, salePrice: 5000)];
    ...
    testWidgets('a failure is explained and the button works again', (tester) async {
      ...
      final button = tester.widget<IconButton>(find.widgetWithIcon(IconButton, Icons.picture_as_pdf_outlined));
