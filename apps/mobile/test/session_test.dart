...
void main() {
  ...
  group('opening the app', () {
    ...
    testWidgets('a session the server refuses (401) sends the user back to sign in', (tester) async {
      final tokens = FakeTokenStorage(access: 'a', refresh: 'r');
  ...
  group('signing in', () {
    testWidgets('quietly prepares the till for offline use, before any screen asks for it', (tester) async {
  ...
  group('signing out', () {
    testWidgets('forgets the offline cache, so the next account never sees this one\'s data', (tester) async {
      final store = MemoryLocalStore()
        ..values['offline_cache:index'] = '["user-1:biz-1|GET /products?"]'
        ..values['offline_cache:entry:user-1:biz-1|GET /products?'] = '{"t":"2026-09-19T10:00:00Z","d":{}}';
