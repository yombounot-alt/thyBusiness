...
import 'package:thy_business/core/offline/offline_providers.dart';
import 'package:thy_business/core/storage/local_store.dart';
import 'package:thy_business/core/theme/formatters.dart';
...
import 'package:thy_business/features/sales/offline/pending_sale.dart';
import 'package:thy_business/features/sales/offline/pending_sales_repository.dart';
...
void main() {
  ...
  group('ringing up a sale with no network', () {
    ...
    testWidgets('the stock shown already counts the sale, so the last unit is not sold twice', (tester) async {
      salesApi.networkDown = true;
      await pump(tester);
    ...
    testWidgets('a banner says so, and opens the list of waiting sales', (tester) async {
      ...
      await tester.pumpAndSettle();
      expect(find.textContaining('Hors ligne · 1 vente enregistrée sur ce téléphone'), findsOneWidget);
    ...
    testWidgets('a refusal from the server is shown to the cashier, not queued', (tester) async {
      ...
      expect(find.text('Confirmer le paiement'), findsOneWidget, reason: 'still on the payment screen');
    ...
    testWidgets('a credit sale is kept with its customer and what is still owed', (tester) async {
      final customers = FakeCustomersApi([
        const Customer(id: 'cust-1', fullName: 'Mamadou Diallo', phone: '+224655555555', currentBalance: 0),
      ]);
      salesApi.networkDown = true;
      await pumpAuthenticatedApp(
        tester,
  ...
  group('sending the waiting sales', () {
    ...
    testWidgets('once the server is back: sent with its real date, and the receipt turns into the real one', (tester) async {
      ...
      expect(find.text('VTE-0001'), findsOneWidget, reason: 'the open receipt moved on to the real sale');
      expect(find.text('Vente enregistrée sur ce téléphone'), findsNothing);
      expect(storedQueue(), isEmpty);
    });

    testWidgets('a sale the server already recorded (its answer was lost) is never sold twice', (tester) async {
    ...
    testWidgets('a sale the server refuses for good is kept, with the reason, until the user decides', (tester) async {
      await queueOneSale(tester);
      salesApi.networkDown = false;
  ...
  group('models', () {
    test('a waiting sale survives being written to the phone and read back', () {
      ...
      final back = PendingSale.fromJson(jsonDecode(jsonEncode(sale.toJson())) as Map<String, dynamic>);
