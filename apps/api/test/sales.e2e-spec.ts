...
describe('Sales / POS (e2e)', () => {
  ...
  const userA = { phone: '+224633333301', password: 'MotDePasse123!', fullName: 'Vendeuse A' };
  const userB = { phone: '+224633333302', password: 'MotDePasse123!', fullName: 'Vendeur B' };
  ...
  beforeAll(async () => {
    ...
    const businessIds = (
      await prisma.business.findMany({ where: { ownerId: { in: users.map((u) => u.id) } } })
    ).map((b) => b.id);
    ...
    await prisma.businessMember.deleteMany({ where: { userId: { in: users.map((u) => u.id) } } });
  ...
  describe('idempotency and credit sales', () => {
    ...
    it('sells only once when the same request arrives twice at the same time', async () => {
      ...
      const send = () =>
        request(server).post('/sales').set('Authorization', `Bearer ${a.accessToken}`).send(body);
  ...
  describe('sales rung up offline and synced later', () => {
    ...
    it('keeps the real date of the sale, on the sale and on its stock movement', async () => {
      ...
      const movements = await request(server)
        .get(`/inventory/movements?productId=${soapId}`)
        .set('Authorization', `Bearer ${a.accessToken}`)
        .expect(200);
