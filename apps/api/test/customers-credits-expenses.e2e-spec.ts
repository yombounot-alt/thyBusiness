...
describe('Customers / Credits / Expenses (e2e)', () => {
  ...
  const userA = { phone: '+224660000001', password: 'MotDePasse123!', fullName: 'Gérante A' };
  const userB = { phone: '+224660000002', password: 'MotDePasse123!', fullName: 'Gérant B' };
  ...
  it('creates a customer', async () => {
    const res = await request(server)
      ...
      .send({ fullName: 'Mamadou Diallo', phone: '+224655555555' })
      .expect(201);
  ...
  it('rejects a credit payment larger than the outstanding debt', async () => {
    await request(server)
      .post(`/customers/${customerId}/credit-payments`)
      .set('Authorization', `Bearer ${a.accessToken}`)
  ...
  describe('expenses', () => {
    ...
    it('reports the total of the whole filtered period, not just the returned page', async () => {
      ...
      const firstPage = await request(server)
        .get('/expenses?page=1&pageSize=1')
        .set('Authorization', `Bearer ${a.accessToken}`)
        .expect(200);
      ...
      const empty = await request(server)
        .get('/expenses?from=2020-01-01&to=2020-01-02')
        .set('Authorization', `Bearer ${a.accessToken}`)
        .expect(200);
  ...
  describe('credits overview', () => {
    it('lists only credits still owed, earliest due date first, undated last', async () => {
      ...
      const res = await request(server)
        .get('/credits?status=outstanding&pageSize=100')
        .set('Authorization', `Bearer ${a.accessToken}`)
        .expect(200);
