...
describe('Products / Categories / Inventory (e2e)', () => {
  ...
  const userA = { phone: '+224622222201', password: 'MotDePasse123!', fullName: 'Commerçante A' };
  const userB = { phone: '+224622222202', password: 'MotDePasse123!', fullName: 'Commerçant B' };
  ...
  async function signupLoginAndCreateBusiness(user: typeof userA, businessName: string) {
    ...
    const noBusinessToken = verifyRes.body.accessToken as string;

    const businessRes = await request(server)
      .post('/businesses')
      .set('Authorization', `Bearer ${noBusinessToken}`)
      .send({ name: businessName, businessType: 'commerce' })
      .expect(201);

    return {
      accessToken: businessRes.body.accessToken as string,
      noBusinessToken,
      businessId: businessRes.body.business.id as string,
    };
  }
  ...
  beforeAll(async () => {
    ...
    for (const phone of [userA.phone, userB.phone]) {
      await prisma.inventoryMovement.deleteMany({
        where: { businessId: { in: await businessIdsFor(phone) } },
      });
    }
    await prisma.product.deleteMany({
      where: { business: { owner: { phone: { in: [userA.phone, userB.phone] } } } },
    });
    await prisma.category.deleteMany({
      where: { business: { owner: { phone: { in: [userA.phone, userB.phone] } } } },
    });
    await prisma.businessMember.deleteMany({
      where: { user: { phone: { in: [userA.phone, userB.phone] } } },
    });
    await prisma.business.deleteMany({
      where: { owner: { phone: { in: [userA.phone, userB.phone] } } },
    });
    await prisma.refreshToken.deleteMany({
      where: { user: { phone: { in: [userA.phone, userB.phone] } } },
    });
    await prisma.otpVerification.deleteMany({
      where: { phone: { in: [userA.phone, userB.phone] } },
    });
    await prisma.user.deleteMany({ where: { phone: { in: [userA.phone, userB.phone] } } });

    async function businessIdsFor(phone: string): Promise<string[]> {
      const user = await prisma.user.findUnique({ where: { phone } });
      if (!user) return [];
      const owned = await prisma.business.findMany({ where: { ownerId: user.id } });
  ...
  it('creates a product with initial stock, crediting an "initial" inventory movement', async () => {
    ...
    const movements = await request(server)
      .get(`/inventory/movements?productId=${productAId}`)
      .set('Authorization', `Bearer ${a.accessToken}`)
      .expect(200);
