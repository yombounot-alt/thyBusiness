...
describe('Direct payments: Orange Money / Mobile Money / merchant code (e2e)', () => {
  ...
  const owner = { phone: '+224624444401', password: 'MotDePasse123!', fullName: 'Propriétaire A' };
  const otherOwner = {
    phone: '+224624444402',
    password: 'MotDePasse123!',
    fullName: 'Propriétaire B',
  };
  const cashierUser = { phone: '+224624444403', password: 'MotDePasse123!', fullName: 'Caissier' };
  ...
  async function signup(user: typeof owner) {
  ...
  async function ownerWithBusiness(user: typeof owner, name: string) {
    ...
    const res = await request(server)
      ...
      .expect(201);
    const dbUser = await prisma.user.findUniqueOrThrow({ where: { phone: user.phone } });
    return {
  ...
  const stockOf = async (token: string, id: string) =>
    Number(
      (await request(server).get(`/products/${id}`).set(auth(token)).expect(200)).body.currentStock,
    );

  const salesCount = (businessId: string) => prisma.sale.count({ where: { businessId } });

  const createMethod = (token: string, body: Record<string, unknown>) =>
    request(server).post('/payment-methods').set(auth(token)).send(body);

  const start = (token: string, body: Record<string, unknown>) =>
    request(server).post('/payments').set(auth(token)).send(body);
  ...
  const submit = (token: string, id: string, body: Record<string, unknown>) =>
    request(server).post(`/payments/${id}/submit`).set(auth(token)).send(body);
  const verify = (token: string, id: string) =>
    request(server).post(`/payments/${id}/verify`).set(auth(token));
  const reject = (token: string, id: string, body: Record<string, unknown> = {}) =>
    request(server).post(`/payments/${id}/reject`).set(auth(token)).send(body);
  const cancel = (token: string, id: string) =>
    request(server).post(`/payments/${id}/cancel`).set(auth(token));
  const read = (token: string, id: string) =>
    request(server).get(`/payments/${id}`).set(auth(token));
  ...
  beforeAll(async () => {
    ...
    // A clean slate for these three users (and everything hanging off their businesses).
    ...
    const w = { businessId: { in: businessIds } };
    await prisma.manualPayment.deleteMany({ where: w });
    await prisma.paymentMethod.deleteMany({ where: w });
    await prisma.creditPayment.deleteMany({ where: w });
    await prisma.customerCredit.deleteMany({ where: w });
    await prisma.payment.deleteMany({ where: w });
    await prisma.saleItem.deleteMany({ where: { sale: w } });
    await prisma.sale.deleteMany({ where: w });
    await prisma.customer.deleteMany({ where: w });
    await prisma.inventoryMovement.deleteMany({ where: w });
  ...
  it('sets up two businesses, a cashier, and products', async () => {
    ...
    await signup(cashierUser);
    const cashierRow = await prisma.user.findUniqueOrThrow({ where: { phone: cashierUser.phone } });
    await prisma.businessMember.create({
      data: { businessId: a.businessId, userId: cashierRow.id, role: 'cashier' },
    });
    const login = await request(server)
      .post('/auth/login')
      .send({ phone: cashierUser.phone, password: cashierUser.password })
      .expect(201);
    cashier = login.body.accessToken;
  ...
  describe('settings: payment methods', () => {
    it('lets the owner add Orange Money, Mobile Money and a merchant code — none exist until then', async () => {
      const empty = await request(server).get('/payment-methods').set(auth(a.token)).expect(200);
    ...
    it('accepts "other", a USSD template, custom instructions — and cleans what was typed', async () => {
      ...
      await request(server)
        .delete(`/payment-methods/${res.body.id}`)
        .set(auth(a.token))
        .expect(204);
    });
    ...
    it('allows several accounts of the same operator, and the owner picks which are active', async () => {
      ...
      const list = await request(server).get('/payment-methods').set(auth(a.token)).expect(200);
      expect(list.body.filter((m: any) => m.provider === 'orange_money')).toHaveLength(2);

      await request(server)
        .patch(`/payment-methods/${second.body.id}`)
        .set(auth(a.token))
        .send({ isActive: false })
        .expect(200);
      // The owner still sees it (to switch it back on); a cashier is only offered the active ones.
      expect(
        (await request(server).get('/payment-methods').set(auth(a.token)).expect(200)).body,
      ).toHaveLength(4);
      ...
      await request(server)
        .get(`/payment-methods/${second.body.id}`)
        .set(auth(cashier))
        .expect(404);

      await request(server)
        .delete(`/payment-methods/${second.body.id}`)
        .set(auth(a.token))
        .expect(204);
    });
    ...
    it('updates partially, clears optional fields when sent empty, and keeps required ones', async () => {
      const res = await request(server)
        .patch(`/payment-methods/${orangeId}`)
        .set(auth(a.token))
    ...
    it('is reserved to the owner for changes, readable by the team, closed to strangers', async () => {
      ...
      await request(server)
        ...
        .send({ isActive: false })
        .expect(403);
      await request(server).delete(`/payment-methods/${orangeId}`).set(auth(cashier)).expect(403);
      await request(server).get(`/payment-methods/${orangeId}`).set(auth(cashier)).expect(200);

      await request(server).get('/payment-methods').expect(401);
      await request(server)
    ...
    it("keeps one business's methods invisible and untouchable for another (404, never 403)", async () => {
      expect(
        (await request(server).get('/payment-methods').set(auth(b.token)).expect(200)).body,
      ).toEqual([]);
      await request(server).get(`/payment-methods/${orangeId}`).set(auth(b.token)).expect(404);
      await request(server)
        .patch(`/payment-methods/${orangeId}`)
        .set(auth(b.token))
        .send({ isActive: false })
        .expect(404);
      await request(server).delete(`/payment-methods/${orangeId}`).set(auth(b.token)).expect(404);
    });

    it('stores a logo after checking the picture itself, and serves it back', async () => {
      await request(server)
        .put(`/payment-methods/${orangeId}/logo`)
        .set(auth(a.token))
      ...
      const logo = await request(server)
        .get(`/payment-methods/${orangeId}/logo`)
        .set(auth(cashier))
        .expect(200);
      ...
      await request(server).get(`/payment-methods/${orangeId}/logo`).set(auth(b.token)).expect(404);
    });

    it('refuses a fake picture, an oversized one, and a logo from a non-owner', async () => {
      await request(server)
        .put(`/payment-methods/${momoId}/logo`)
        .set(auth(a.token))
      ...
      await request(server)
        ...
        .attach('file', PNG, { filename: 'logo.png', contentType: 'image/png' })
        .expect(403);
      await request(server).put(`/payment-methods/${momoId}/logo`).set(auth(a.token)).expect(400); // no file

      expect(
        (await request(server).get(`/payment-methods/${momoId}`).set(auth(a.token)).expect(200))
          .body.hasLogo,
      ).toBe(false);
    });
    ...
    it('removes the logo', async () => {
      const res = await request(server)
        .delete(`/payment-methods/${orangeId}/logo`)
        .set(auth(a.token))
        .expect(200);
      expect(res.body.hasLogo).toBe(false);
      await request(server).get(`/payment-methods/${orangeId}/logo`).set(auth(a.token)).expect(404);
    });
  });
  ...
  describe('the customer chooses a payment method', () => {
    ...
    it('is idempotent: the same key returns the same payment (double tap, refresh)', async () => {
      ...
      expect(
        await prisma.manualPayment.count({
          where: { businessId: a.businessId, clientRequestId: key },
        }),
      ).toBe(1);
    });
    ...
    it('keeps a used method (deactivate instead of delete), and removes an unused one', async () => {
      await request(server).delete(`/payment-methods/${orangeId}`).set(auth(a.token)).expect(409);
      const fresh = await createMethod(a.token, {
        provider: 'other',
        phoneNumber: '620000003',
      }).expect(201);
      await request(server)
        .delete(`/payment-methods/${fresh.body.id}`)
        .set(auth(a.token))
        .expect(204);
    });
  });
  ...
  describe('"J’ai effectué le paiement": the declaration', () => {
    ...
    it('refuses a wrong amount — the exact amount is required', async () => {
      ...
      for (const amountSent of [9999, 10001, 5000, 0, -10000]) {
        ...
        if (amountSent === 5000) expect(res.body.message).toContain('exactement');
      }
    ...
    it('needs a login, and hides another business’s payment (404)', async () => {
      const p = await newPayment(2);
      await request(server).post(`/payments/${p.id}/submit`).send(declaration(10000)).expect(401);
  ...
  describe('proof of payment', () => {
    it('attaches a picture to a declared payment and serves it back to the business only', async () => {
      ...
      await request(server).get(`/payments/${p.id}/proof`).set(auth(b.token)).expect(404);
      await request(server).get(`/payments/${p.id}/proof`).expect(401);
    });
    ...
    it('refuses fake, oversized and missing files', async () => {
      ...
      await request(server)
        ...
        .expect(413);
      await request(server).put(`/payments/${p.id}/proof`).set(auth(a.token)).expect(400);
    ...
    it('cannot be changed once the payment is decided, and can be removed before', async () => {
      ...
      const removed = await request(server)
        .delete(`/payments/${p.id}/proof`)
        .set(auth(a.token))
        .expect(200);
      expect(removed.body.hasProof).toBe(false);
      await request(server).get(`/payments/${p.id}/proof`).set(auth(a.token)).expect(404);
  ...
  describe('the owner verifies', () => {
    it('records the sale once the owner validates: paid, stock down, reference kept', async () => {
      ...
      const row = await prisma.manualPayment.findUniqueOrThrow({ where: { id: p.id } });
      expect(row.verifiedBy).toBe(a.userId);
    });
    ...
    it('never loses a verified payment when the sale cannot be recorded: verify again to finish', async () => {
      ...
      await submit(a.token, p.id, declaration(5000)).expect(201);
      await prisma.customer.delete({ where: { id: customer.body.id } }); // the sale can no longer be recorded
      ...
      await prisma.customer.create({
        data: { id: customer.body.id, businessId: a.businessId, fullName: 'Client éphémère' },
      });
      ...
      expect(
        await prisma.sale.count({
          where: {
            clientRequestId: (await prisma.manualPayment.findUniqueOrThrow({ where: { id: p.id } }))
              .clientRequestId,
          },
        }),
      ).toBe(1);
    ...
    it('hides another business’s payment from the verification (404)', async () => {
      ...
      expect((await read(a.token, p.id).expect(200)).body.status).toBe('submitted');
      await request(server).post(`/payments/${p.id}/verify`).expect(401);
    });
  });
  ...
  describe('the owner refuses', () => {
    it('records the reason, shows it, and sells nothing', async () => {
      ...
      const row = await prisma.manualPayment.findUniqueOrThrow({ where: { id: p.id } });
      expect(row.rejectedBy).toBe(a.userId);
    });
  ...
  describe('cancelling, and unpaid orders', () => {
    ...
    it('cannot cancel a validated payment (cancel the sale instead)', async () => {
      ...
      expect(res.body.message).toContain('annulez la vente');
    });
  ...
  describe('lists, summary and isolation', () => {
    it('lists the business’s payments, newest first, filtered by status and paginated', async () => {
      const all = await request(server).get('/payments').set(auth(a.token)).expect(200);
      ...
      const submitted = await request(server)
        .get('/payments?status=submitted&pageSize=100')
        .set(auth(a.token))
        .expect(200);
      ...
      const page = await request(server)
        .get('/payments?pageSize=2&page=2')
        .set(auth(a.token))
        .expect(200);
      expect(page.body.items).toHaveLength(2);
      expect(page.body.page).toBe(2);

      await request(server).get('/payments?status=paid').set(auth(a.token)).expect(400);
      await request(server).get('/payments?pageSize=0').set(auth(a.token)).expect(400);
    });

    it('counts payments per status (the "à vérifier" badge)', async () => {
      const summary = await request(server).get('/payments/summary').set(auth(a.token)).expect(200);
      ...
      for (const status of Object.keys(summary.body)) {
        const real = await prisma.manualPayment.count({
          where: { businessId: a.businessId, status },
        });
        expect(summary.body[status]).toBe(real);
      }
    });

    it('shows nothing of another business — lists, counts, details, decisions', async () => {
      const list = await request(server).get('/payments').set(auth(b.token)).expect(200);
      const ids = list.body.items.map((i: any) => i.id);
      const aPayment = await prisma.manualPayment.findFirstOrThrow({
        where: { businessId: a.businessId },
      });
      expect(ids).not.toContain(aPayment.id);

      const summary = await request(server).get('/payments/summary').set(auth(b.token)).expect(200);
    ...
    it('needs a login everywhere', async () => {
      ...
      await request(server).get('/payments/summary').expect(401);
      await request(server)
        .get(`/payments/${(await prisma.manualPayment.findFirstOrThrow()).id}`)
        .expect(401);
    });
  });
});
