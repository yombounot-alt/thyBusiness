...
describe('Product images (e2e)', () => {
  ...
  const userA = { phone: '+224661000001', password: 'MotDePasse123!', fullName: 'Photographe A' };
  const userB = { phone: '+224661000002', password: 'MotDePasse123!', fullName: 'Photographe B' };
  ...
  beforeAll(async () => {
    ...
    await prisma.product.deleteMany({ where: { business: { owner: { phone: { in: phones } } } } });
    await prisma.businessMember.deleteMany({ where: { user: { phone: { in: phones } } } });
    await prisma.business.deleteMany({ where: { owner: { phone: { in: phones } } } });
  ...
  it('has no photo yet: reading it is a 404', async () => {
    await request(server).get(`/products/${productId}/image`).set(auth(a)).expect(404);
  });
  ...
  it('stores a PNG and serves the same bytes back with safe headers', async () => {
    ...
    const img = await request(server).get(`/products/${productId}/image`).set(auth(a)).expect(200);
    expect(img.headers['content-type']).toBe('image/png');
    expect(img.headers['cache-control']).toBe('private, max-age=86400');
    expect(img.headers['x-content-type-options']).toBe('nosniff');
    expect(Buffer.compare(img.body, PNG)).toBe(0);

    const product = await request(server).get(`/products/${productId}`).set(auth(a)).expect(200);
  ...
  it('replacing the photo serves the new one and removes the old file', async () => {
    const before = await request(server).get(`/products/${productId}`).set(auth(a)).expect(200);
  ...
  it('rejects files that are not JPEG/PNG/WebP, whatever their name or declared type', async () => {
    ...
    const after = await request(server).get(`/products/${productId}`).set(auth(a)).expect(200);
    expect(after.body.imageKey).toBe(before.body.imageKey);
  });

  it('rejects a request without a file', async () => {
    await request(server).put(`/products/${productId}/image`).set(auth(a)).expect(400);
  });
  ...
  it('requires authentication', async () => {
    await request(server).get(`/products/${productId}/image`).expect(401);
    await request(server)
      .put(`/products/${productId}/image`)
      .attach('file', PNG, { filename: 'x.png' })
      .expect(401);
    await request(server).delete(`/products/${productId}/image`).expect(401);
  });

  it("keeps business A's photo invisible and untouchable for business B (404, not 403)", async () => {
    const before = await request(server).get(`/products/${productId}`).set(auth(a)).expect(200);

    await request(server).get(`/products/${productId}/image`).set(auth(b)).expect(404);
    await upload(b, productId, PNG).expect(404);
    await request(server).delete(`/products/${productId}/image`).set(auth(b)).expect(404);
  ...
  it('removes the photo: key cleared, file deleted, reading it is a 404 again', async () => {
    ...
    await request(server).delete(`/products/${productId}/image`).set(auth(a)).expect(200);
  });
});
