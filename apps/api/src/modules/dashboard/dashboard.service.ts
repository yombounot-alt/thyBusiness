...
export class DashboardService {
  ...
  async getSummary(businessId: string, query: DashboardPeriodQuery) {
    ...
    const lowStockCount = (await this.products.lowStock(businessId)).length;

    const outstandingAgg = await this.prisma.customerCredit.aggregate({
    ...
    const overdueCreditsCount = await this.prisma.customerCredit.count({
      where: {
        businessId,
