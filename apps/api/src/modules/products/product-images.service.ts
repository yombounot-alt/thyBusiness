...
import { STORAGE_PROVIDER, StoragePort } from '../storage/storage.port';
...
export class ProductImagesService {
  ...
  async remove(businessId: string, productId: string) {
    const product = await this.find(businessId, productId);
    if (!product.imageKey) return this.withCategory(productId);

    await this.prisma.product.update({ where: { id: productId }, data: { imageKey: null } });
    await this.discard(product.imageKey);
    return this.withCategory(productId);
  }
  ...
  private withCategory(productId: string) {
