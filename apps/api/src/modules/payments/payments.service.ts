...
import { PROOF_MAX_BYTES, PaymentFilesService } from './payment-files.service';
import {
  PAYMENT_STATUSES,
  PaymentProvider,
  PaymentStatus,
...
function isUniqueViolation(error: unknown, column?: string): boolean {
  ...
  return ((error.meta?.target as string[] | string | undefined) ?? []).includes(column);
}
...
export class PaymentsService {
  ...
  async submit(businessId: string, id: string, dto: SubmitPaymentDto) {
    ...
    const clash = await this.prisma.manualPayment.findFirst({
      where: {
        businessId,
    ...
    let count: number;
    try {
      ({ count } = await this.prisma.manualPayment.updateMany({
  ...
  async reject(businessId: string, userId: string, id: string, dto: RejectPaymentDto) {
    ...
    const { count } = await this.prisma.manualPayment.updateMany({
      where: { id, status: 'submitted' },
      data: {
        status: 'rejected',
  ...
  /** Stops a payment that will not happen. Cancelling one that was already declared is the owner's call. */
  async cancel(businessId: string, role: string | null, id: string) {
    const payment = await this.findOwned(businessId, id);
    ...
    if (payment.status === 'submitted' && role !== 'owner') {
  ...
  async summary(businessId: string) {
    const groups = await this.prisma.manualPayment.groupBy({
      by: ['status'],
      where: { businessId },
      _count: { _all: true },
    });
  ...
  async removeProof(businessId: string, id: string) {
    ...
    const updated = await this.prisma.manualPayment.update({
      where: { id },
      data: { proofKey: null },
      include: { method: true },
    });
  ...
  private async recordSale(payment: PaymentWithMethod): Promise<PaymentWithMethod> {
    ...
    try {
      ...
      return await this.prisma.manualPayment.update({
        where: { id: payment.id },
        data: { saleId: sale.id },
        include: { method: true },
      });
  ...
  private findByRequestId(businessId: string, clientRequestId: string) {
    return this.prisma.manualPayment.findUnique({
