...
import { LOGO_MAX_BYTES, PaymentFilesService } from './payment-files.service';
import {
  PROVIDER_LABEL,
  PaymentProvider,
  cleanLine,
...
export class PaymentMethodsService {
  ...
  /** The owner sees every method (to manage them); anyone else only the ones offered to customers. */
  async list(businessId: string, role: string | null) {
  ...
  async findOne(businessId: string, id: string, role: string | null) {
    const method = await this.find(businessId, id);
    if (role !== 'owner' && !method.isActive) {
  ...
  async remove(businessId: string, id: string) {
    const method = await this.find(businessId, id);
    const used = await this.prisma.manualPayment.count({ where: { paymentMethodId: id } });
    if (used > 0) {
  ...
  async setLogo(businessId: string, id: string, file: Buffer) {
    ...
    try {
      updated = await this.prisma.paymentMethod.update({ where: { id }, data: { logoKey: key } });
    } catch (error) {
  ...
  async readLogo(businessId: string, id: string, role: string | null) {
    const method = await this.find(businessId, id);
    if ((role !== 'owner' && !method.isActive) || !method.logoKey) {
  ...
  private async find(businessId: string, id: string) {
    const method = await this.prisma.paymentMethod.findFirst({ where: { id, businessId } });
